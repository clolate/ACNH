...
const sortInsects = () => {
  ...
  for (const m of months) {
    const civs = d3.select(`#${m}`).selectAll(".insect");
...
const readCsv = async () => {
  for (const m of months) {
    month = m;
    // Parse the Data
    await d3.csv("datasets/insects.csv", parse).then((data) => {
      gData[m] = data;
      ...
      let color = d3.scaleOrdinal(d3.schemePaired ).domain(regions);
